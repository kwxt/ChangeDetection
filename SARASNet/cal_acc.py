...
for file in tqdm.tqdm(glob.glob('./test_dataset_com/A/*')):
    filename = file.split('\\')
    filename = filename[-1]
    testCase1_01 = './test_dataset_com/A/' + filename
    testCase1_02 = './test_dataset_com/B/' + filename
    gt           = './test_dataset_com/label/' +  filename
    ...
    for i in range(0, h1, patch):
        for j in range(0, w1, patch):
            ...
            with torch.no_grad():
              inputs1,inputs2,targets = Variable(inputs1.unsqueeze(0)),Variable(inputs2.unsqueeze(0)) ,Variable(targets)
