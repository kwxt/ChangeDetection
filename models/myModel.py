...
class ContextEncoding(nn.Module):
    def __init__(self, in_channels, context_channels=64):
    ...
    def forward(self, x):
        # 获取全局上下文信息
        context = torch.mean(x, dim=(2, 3), keepdim=True)  # 平均池化得到上下文
...
class CCAM(nn.Module):
    def __init__(self, in_channels, context_channels=64, reduction=16):
        ...
        # 确保最终输出通道数为64
        self.output_conv = nn.Conv2d(in_channels, 64, kernel_size=1)
...
# 替换MobileNet为EfficientNet-B0前五个模块
class LCD_Net(nn.Module):
    def __init__(self):
        super(LCD_Net, self).__init__()

        # 使用EfficientNet-B0的前五个模块替换MobileNetV2
        ...
        self.decoder = nn.Sequential(CCAM(in_channels=472), nn.Conv2d(64, 1, 1))

        self.decoder_4 = CCAM(in_channels=64 * 2 + 1)

        self.decoder_3 = CCAM(64 * 3 + 1)
        self.decoder_2 = CCAM(64 * 3 + 1)
        self.decoder_1 = CCAM(64 * 3 + 1)
        self.decoder_final = nn.Sequential(CCAM(64), nn.Conv2d(64, 1, 1))
