...
from models.Model import SNUNet_CCAM
...
def load_model(opt, device):
    ...
    #model=SNUNet_CCAM(opt.num_channel, 2).to(device)
    #model = SiamUnet_diff(opt.num_channel, 2).to(device)
    #model = Siam_NestedUNet_Conc(opt.num_channel, 2).to(device)
