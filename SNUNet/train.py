...
if __name__ == '__main__':
    ...
    for epoch in range(opt.epochs):
        ...
        for batch_img1, batch_img2, labels in tbar:
            ...
            #print('cd_preds')
            #print([x.shape for x in cd_preds])

            cd_loss = criterion(cd_preds, labels)
            print('labels.uique:')
            print(labels.unique())
            labels_numpy = labels.cpu().numpy()
            labels_grayscale = (labels_numpy * 255).astype(np.uint8)
            for i, img in enumerate(labels_grayscale):
                cv2.imwrite(f'labels_imge_{i}.png', img)
            loss = cd_loss
            loss.backward()
            optimizer.step()
            ...
            #print(cd_preds.shape)
            #print(cd_preds)
            cd_preds_numpy=cd_preds.cpu().numpy()
            cd_preds_grayscale=(cd_preds_numpy*255).astype(np.uint8)
            for i , img in enumerate(cd_preds_grayscale):
                cv2.imwrite(f'prediction_imge_{i}.png',img)
        ...
        with torch.no_grad():
            ...
            if ((mean_val_metrics['cd_precisions'] > best_metrics['cd_precisions'])
                ...
                # Save model and log
                if not os.path.exists('./tmp_Com'):
                    os.mkdir('./tmp_Com')
                with open('./tmp_Com/metadata_epoch_' + str(epoch) + '.json', 'w') as fout:
                    json.dump(metadata, fout)

                torch.save(model, './tmp_Com/checkpoint_epoch_'+str(epoch)+'.pt')
