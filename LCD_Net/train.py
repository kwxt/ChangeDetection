...
import numpy as np
import utilss.visualization as visual
from utilss import data_loader
from tqdm import tqdm
import random
from utilss.metrics import Evaluator

from network.LCD_Net import LCD_Net
import time
start=time.time()
import time
...
def save_checkpoint(epoch, model, optimizer, scheduler, best_iou, save_path):
    checkpoint = {
        'epoch': epoch,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_path', type=str,
                        default='./outputs_new_2')
    parser.add_argument('--model_name', type=str, default='LCD-Net',required=True, help='Name of the model')
    ...
    if opt.data_name == 'LEVIR':
        opt.train_root = '../LEVIR/train/'
        opt.val_root = '../LEVIR/val/'
    elif opt.data_name == 'Com':
        opt.train_root = "../datasets/train/"
        opt.val_root = "../datasets/val/"
    elif opt.data_name == 'SYSU':
        opt.train_root = '../SYSU/train/'
        opt.val_root = '../SYSU/val/'
    elif opt.data_name == 'CDD':
        opt.train_root = '../CDD/train/'
        opt.val_root = '../CDD/val/'
    ...
    model = LCD_Net().cuda()
        ...
        #    '/root/autodl-tmp/LCD_Net/output/S2Looking/CGNet_best_iou.pth')
...
# 加载断点
    '''
    start_epoch, best_iou = load_checkpoint(model, optimizer, lr_scheduler, save_path)
    ...
    for epoch in range(start_epoch, opt.epoch + 1):
        ...
        # 每个 epoch 保存一次断点
        save_checkpoint(epoch, model, optimizer, lr_scheduler, best_iou, save_path)
'''
    end=time.time()
