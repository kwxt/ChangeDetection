...
from network.new_1 import LCD_Net
import time
start=time.time()
...
if __name__ == '__main__':
    ...
    if opt.data_name == 'LEVIR':
        opt.test_root = '../LEVIR/test/'
    elif opt.data_name == 'Com':
        opt.test_root = "../datasets/test/"
    elif opt.data_name == 'SYSU':
        opt.test_root = '../SYSU/test/'
    elif opt.data_name == 'CDD':
        opt.test_root = '../CDD/test/'
    ...
    if opt.model_name == 'LCD-Net':
        model = LCD_Net().cuda()
    else:
        raise ValueError(f"Unsupported model_name: {opt.model_name}")

    opt.load = './outputs_new_1SYSU/LCD-Net_best_iou.pth'
    # 检查模型加载路径
    if opt.load is not None:
        if os.path.exists(opt.load):
            ...
            raise FileNotFoundError(f"Model file not found at {opt.load}")
